        ...
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-[1.25fr_1fr_1fr_1fr]">
            ...
            <div className="mt-6 flex items-center gap-3">
              {socialLinks.map(({ label, href, icon: Icon }) => (
                <a
                  ...
                  className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white/70 transition duration-300 hover:bg-[#41C7D7] hover:text-white"
                  aria-label={label}
                >
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3 text-sm text-white/70">
              {services.map((s) => (
                <li key={s.slug}>
                  <Link href={`/services/${s.slug}`} className="transition duration-300 hover:text-[#41C7D7]">
                    {s.title}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3 text-sm text-white/70">
              {quickLinks.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="transition duration-300 hover:text-[#41C7D7]">
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="mt-5 space-y-4 text-sm text-white/70">
              <li className="flex items-start gap-3">
                ...
                <a href="tel:+966595394940" className="transition duration-300 hover:text-[#41C7D7]">
                  +966 59 539 4940
                </a>
              </li>
              <li className="flex items-start gap-3">
                <Mail className="mt-1 h-4 w-4 shrink-0 text-[#41C7D7]" aria-hidden="true" />
                <a href="mailto:hello@promksa.com" className="transition duration-300 hover:text-[#41C7D7]">
                  hello@promksa.com
                </a>
              </li>
              <li className="flex items-start gap-3">
